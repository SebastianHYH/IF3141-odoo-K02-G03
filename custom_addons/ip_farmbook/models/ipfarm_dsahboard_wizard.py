...
class IPFarmDashboardWizard(models.TransientModel):
  ...
  _description = 'Dashboard Analitik Operasional'
  ...
  def _compute_kpi(self):
    for wizard in self:
      ...
      wizard.stok_produk_terkini = products[0].stok if products else 0.0
      ...
      wizard.prediksi_html = wizard._generate_prediction_html()
      
      
      
  def _generate_prediction_html(self):
    self.ensure_one()
    panen = self.env['ipfarm.panen'].search([('batch_id.tanggal_tanam', '!=', False)])
    durations = [
      (p.tanggal_panen - p.batch_id.tanggal_tanam).days 
      for p in panen 
      if p.tanggal_panen and p.batch_id.tanggal_tanam
    ]
    ...
    curr_stock = self.produk_id.stok_tersedia if self.produk_id else sum(self.env['ipfarm.produk'].search([]).mapped('stok_tersedia'))
    projected_7_days = curr_stock + ((avg_in - avg_out) * 7)
    
    if avg_duration:
      panen_text = _("Rata-rata durasi panen: %.1f hari") % avg_duration
    else:
      panen_text = _("Belum ada data panen historis untuk menghitung rata-rata durasi panen.")
      
    stock_text = _("Prediksi stok 7 hari: %.2f berdasarkan rata-rata stok masuk %.2f/hari dan keluar %.2f/hari.") % (
      projected_7_days,
      avg_in,
      avg_out,
    )
      
    return "<p>%s</p><p>%s</p>" % (panen_text, stock_text)
